...
ConstructedPath = collections.namedtuple(
    "ConstructedPath", ["obj_name", "rel_path", "id", "type", "content"]
)
...
def construct_file_paths(collection_data, collection_path):
    ...
    def parse_obj(obj_data, suffix=".alias", own_folder=True):
        ...
        constructed = ConstructedPath(
            folder_name,
            curpath,
            obj_data["_id"],
            suffix.strip("."),
            content=obj_data["code"],
        )
    ...
    for listed_gvar in ids:
        for i, gvar_name in enumerate(gvar_names):
            if gvar_name.startswith(listed_gvar):
                ...
                gvar = ConstructedPath(
                    gvar_desc if gvar_desc else listed_gvar,
                    files[i],
                    listed_gvar,
                    "gvar",
                    content,
    ...
    for collection in collection_ids:
        ...
        for obj in modified_objects:
            ...
            if result is None:
                ...
                continue
            print(
                f"- Updated {obj.type} {obj.obj_name} ({obj.id})"
